...
<section id="main_banner">
    <div class="container">
        <div class="row">
            <div class="col-12 col-lg-9 col-xl-6 main_banner_content">
                <h3><?$APPLICATION->IncludeComponent(
                    ...
                    Array(
                        ...
                        "EDIT_TEMPLATE" => "",
                        "PATH" => str_replace("//", "/", SITE_DIR . "/inc/main_banner_head.php")
                    )
                );?></h3>
                <p><?$APPLICATION->IncludeComponent(
                    ...
                    Array(
                        ...
                        "AREA_FILE_SUFFIX" => "inc",
                        "EDIT_TEMPLATE" => "",
                        "PATH" => str_replace("//", "/", SITE_DIR . "/inc/main_banner_text.php")
                    )
                );?></p>
                    ...
                    <form  enctype="multipart/form-data" action="<?=POST_FORM_ACTION_URI?>" method="POST" id="prv_feedback_form_<?=$arResult["PARAMS_HASH"];?>">
                        ...
                        <div class="row">
                            <?foreach ($arResult['FIELDS'] as $field):?>
                                <div class="col-12 col-sm-4 main_banner_input_block">
                                    <?switch ($field['PROPERTY_TYPE']) {
                                        ...
                                        case 'S':?>
                                            <?if ($field["USER_TYPE"] == 'HTML'): ?>
                                                <p>
                                                    <label for="<?=$field["CODE"] . '_' . $arResult["PARAMS_HASH"];?>"><?=$field["HINT"];?></label>
                                                    <textarea 
                                            ...
                                            <?else:?>
                                                    <input 
                                                        ...
                                                        name="<?=$field["CODE"];?>" 
                                                        type="text" 
                                                        placeholder="<?=($field["CODE"] == 'PHONE') ? 'mask_phone_number' : '';?>
<?//=$field["HINT"];?>
"
                                                        value="<?=$field["DEFAULT_VALUE"];?>"
                                                        class="<?=($field["CODE"] == 'PHONE') ? 'mask_phone_number' : '';?> h inp_text"
                                                        <?=($field["IS_REQUIRED"] == 'Y') ? 'required' : '';?>
                                            ...
                                            break;
                                        case 'F':?>
                                            <p><?=$field["HINT"];?></p>col-10 callback_input
                                            <p>
                                                <input 
                                                    id="<?=$field["CODE"] . '_' . $arResult["PARAMS_HASH"];?>"
                                                    name="<?=$field["CODE"];?>" 
                            ...
                            <div class="col-12 col-sm-4 main_banner_input_block">
                                <input type="submit" class="btn main_banner_btn" name="submit" value="<?=GetMessage("PRV_UFEEDBACK_CONTACTS_SUBMIT");?>">
                            </div>
                        </div>
